#include <iostream>
int main()
{
    /*
    - std là viết tắt của từ: standard
    - cout: character output
    - cin: character input
    - endl: end line
    */

    /* Các kiểu dữ liệu cơ bản và giới hạn
    Ví dụ: x thuộc Z, điều kiện: 0 <= x <= 10
                                => min của x = 0
                                => max của x = 10
    - Số nguyên: 0, 1, 2, 3, -1, -2, -3
    - Giới hạn của kiểu dữ liệu int là bao nhiên min, max
    - 1 byte = 8 bits  (0110 0010)
    - int (4 bytes) (32 bits) =>          -2^31 <= int <= 2^31 - 1
    - unsigned (4 bytes) (32 bits)            0  <= unsigned <= 2^32
    - int32_t (32 bits)                     -2^31 <= int32_t <= 2^31 - 1
    - int8_t (8 bits)                       -2^7 <= int8_t <= 2^7 - 1


    Ví dụ: x thuộc R
    - Số thực: 1.0, 2.5, 4.9, 7.9, 9.9, 9.12345678901241234
    - float (4 bytes)  (6-7)                     1.5 × 10^−45 <= float <= 3.4 × 10^38.
    - double (8 bytes)  (15)                     5.0 × 10^−324 <= double <= 1.7 × 10^308

    Ví dụ: đúng hoặc sai => Mệnh đề Chỉ đúng hoặc sai
    - 	bool: boolean (1 byte) => true (1) or false (0)
    */
    int a = 5;
    int b = 3;

    int sum = a + b;      //  8
    int minus = a - b;    // 2
    int multiple = a * b; // 15
    int divide = a / b;   // 1.6666 => 1

    int chia_co_du = a % b; // 2

    std::cout << "Value of sum:" << sum << std::endl;
    std::cout << "Value of minus:" << minus << std::endl;
    std::cout << "Value of multiple:" << multiple << std::endl;
    std::cout << "Value of divide:" << divide << std::endl;
    std::cout << "Value of chia_co_du:" << chia_co_du << std::endl;

    return 0;
}
